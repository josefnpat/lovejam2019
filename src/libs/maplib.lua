local mathlib = require"libs.mathlib"
local tilelib = require"libs.tilelib"
...
function maplib.new(init)
  ...
  self.findRandomEmptyTile = maplib.findRandomEmptyTile
...
function maplib:findRandomEmptyTile()
  while true do
    local x = math.random(#self._data)
    local y = math.random(#self._data[x])
    local tile = self:getTile(x,y)
    if tile and tile:getType() == 0 and tile:getMonster() == nil then
      return x,y
    end
  end
end
